...
if (isset($_POST['submit'])) {
 ...
 if (!empty($date1) && !empty($date2)) {
  ...
  $q = mysqli_query($conn, "SELECT * FROM tb_pendaftaran WHERE tgl_daftar BETWEEN '$date1' and '$date2'"); 
 } else {
  // perintah tampil semua data
  $q = mysqli_query($conn, "SELECT * FROM tb_pendaftaran LEFT JOIN tb_pasien USING (no_rm) ORDER BY id_daftar"); 
 }
} else {
 // perintah tampil semua data
 $q = mysqli_query($conn, "SELECT * FROM tb_pendaftaran LEFT JOIN tb_pasien USING (no_rm) ORDER BY id_daftar");
...
<head>
    ...
    <title>Laporan Pendaftaran Pasien - Klinik Apotek</title>
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">Laporan Pendaftaran Pasien</h6>
                        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        ...
                                        $nomor = 1;
                                        while ($pn = $q->fetch_assoc()) {
                                        ?>
                                        <tr>
                                            <td><?= $nomor++ ?></td>
                                            <td><?= date('d-M-Y', strtotime($pn['tgl_daftar'])) ?></td>
                                            <td><?= $pn['no_daftar']; ?></td>
                                            <td><?= $pn['no_rm']; ?> / <?= $pn['nm_pasien']; ?></td>
                                            <td><?= $pn['tgl_janji']; ?>, <?= $pn['jam_janji']; ?></td>
                                            <td><?= $pn['keluhan']; ?></td>
                                            <td align="center"><a href="pendaftaran/pendaftaran_cetak.php?idp=<?= $pn['id_daftar']; ?>" class="btn btn-outline-info"><i class="fas fa-print"></i></td></a>
                                        </tr>
                                        <?php } ?>
                                    </tbody>
                                </table>
