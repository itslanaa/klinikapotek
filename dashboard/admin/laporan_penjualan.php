...
if (isset($_POST['submit'])) {
 ...
 if (!empty($date1) && !empty($date2)) {
  ...
  $q = mysqli_query($conn, "SELECT * FROM tb_penjualan WHERE tgl_penjualan BETWEEN '$date1' and '$date2'"); 
 } else {
  // perintah tampil semua data
  $q = mysqli_query($conn, "SELECT * FROM tb_penjualan LEFT JOIN tb_obat USING (kd_obat) ORDER BY no_penjualan"); 
 }
} else {
 // perintah tampil semua data
 $q = mysqli_query($conn, "SELECT * FROM tb_penjualan LEFT JOIN tb_obat USING (kd_obat) ORDER BY no_penjualan");
...
<head>
    ...
    <title>Laporan Penjualan Obat - Klinik Apotek</title>
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">Laporan Penjualan Obat</h6>
                        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
										...
										$nomor = 1;
										while ($pj = $q->fetch_assoc()) {
										?>
                                        <tr>
                                            <td><?= $nomor++ ?></td>
                                            <td><?= $pj['no_penjualan']; ?></td>
                                            <td><?= $pj['kd_obat']; ?> / <?= $pj['nm_obat']; ?></td>
                                            <td><?= date('d-M-Y', strtotime($pj['tgl_penjualan'])) ?></td>
                                            <td><?= $pj['pelanggan']; ?></td>
                                            <td><?= $pj['keterangan']; ?></td>
                                            <td><?= $pj['uang_bayar']; ?></td>
                                            <td align="center"><a href="penjualan/penjualan_nota.php?id=<?= $pj['no_penjualan']; ?>" class="btn btn-outline-info"><i class="fas fa-print"></i></td></a>
                                        </tr>
                                        <?php } ?>
                                    </tbody>
                                </table>
